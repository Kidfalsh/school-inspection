...
<template>
  <div class="warningDetail" ref="warningDetail">
    ...
    <div class="header" ref="header">
      ...
      <div>
        ...
        <div style="padding-left:20px;min-height:30px;margin-top:10px;">
          <span  style="font-size:12px;color:#5db1b3;">{{yjxx.xsmcs}}</span>
        </div>
      </div>
    </div>
    <div class="container" :style="{height:offHeight}" ref="container">
      <div class="wrap leave-info">
        ...
        <div class="form-item" v-if="saveData.sjjtbx=='法定传染病'&&fdcrbChecked[13]">
          ...
          <input v-model="saveData.qtcrb"  focus="setPos" @blur="setBack" style="padding-right:25px;" type="text" placeholder="请输入">
        </div>
        <!-- 配置其他疑似聚集性事件-->
        <div class="form-item" v-if="saveData.sjjtbx=='其他疑似聚集性事件'">
          <div class="desc">其他疑似聚集性事件</div>
          <input v-model="saveData.qtysjjxsj"  focus="setPos" @blur="setBack" style="padding-right:25px;" type="text" placeholder="请输入">
        ...
        <div class="form-item" v-if="csChecked[2]">
          ...
          <input v-model="saveData.tkcs"  focus="setPos" @blur="setBack" style="padding-right:25px;" type="text" placeholder="请输入停课措施">
        </div>
        <!-- 区疾控已采样检测 -->
        <div class="form-item" v-if="csChecked[7]">
          <div class="desc">检测结果</div>
          <input v-model="saveData.qjkcyjcjg"  focus="setPos" @blur="setBack" style="padding-right:25px;" type="text" placeholder="请输入区疾控采样检测结果">
        </div>
        <!-- 其他 -->
        <div class="form-item" v-if="csChecked[8]">
          <div class="desc">其他措施</div>
          <input v-model="saveData.qtcs"  focus="setPos" @blur="setBack" style="padding-right:25px;" type="text" placeholder="请输入其他措施">
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted(){
    ...
    //获取contain 的实际高度并赋值
    this.offHeight = this.$refs.warningDetail.getBoundingClientRect().height - this.$refs.header.getBoundingClientRect().height - 96 +'px'
    window.onresize = () => {
  ...
  methods:{
    ...
    initSavedata(){
      ...
      let sfysjjxsj='',sftfggwssj='',sjjtbx='';
      //是否疑似聚集性事件
      if(this.saveData.sfysjjxsj){
        if(this.saveData.sfysjjxsj=='是'){
          sfysjjxsj = '1'
        }else if(this.saveData.sfysjjxsj=='否'){
          sfysjjxsj = '0'
        }else{
          sfysjjxsj = '2'
        }
      ...
      //是否突发公共卫生事件
      if(this.saveData.sftfggwssj){
        if(this.saveData.sftfggwssj=='是'){
          sftfggwssj = '1'
        }else if(this.saveData.sftfggwssj=='否'){
          sftfggwssj = '0'
        }else{
          sftfggwssj = '2'
        }
      ...
      //事件具体表现
      if(this.saveData.sjjtbx){
        if(this.saveData.sjjtbx=='法定传染病'){
          sjjtbx = '1'
        }else{
          sjjtbx = '2'
        }
      ...
      let params={
        ...
        sfysjjxsj:sfysjjxsj, //是否疑似聚集性事件
        sftfggwssj:sftfggwssj,//是否突发公共卫生事件
        sjjtbx:sjjtbx,//事件具体表现
